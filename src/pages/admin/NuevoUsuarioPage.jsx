...
export default function NuevoUsuarioPage() {
  ...
  const { isSuperAdmin, sedeId: miSedeId } = useAuth()
  ...
  useEffect(() => {
    rolesService.getAll()
      .then(data => {
        ...
        const filtrados = isSuperAdmin
          ? lista.filter(r => r.nombre === 'administrador')
          : lista.filter(r => r.nombre !== 'administrador')
        setRoles(filtrados.map(r => ({ value: r.id, label: r.nombre })))
      })
  ...
  const handleRolChange = async (rolId) => {
    if (!rolId) { setPermisosRol([]); return }
    try {
      const perms = await rolesService.getPermisos(rolId)
  ...
  const handleGuardar = async (data) => {
    ...
    try {
      await usuariosService.crear({
        ...
        ...(isSuperAdmin ? (data.sedeId ? { sedeId: data.sedeId } : {}) : (miSedeId ? { sedeId: miSedeId } : {})),
      })
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="max-w-3xl mx-auto w-full space-y-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 page-title">Crear Nuevo Usuario</h1>
            <p className="text-sm text-gray-500 mt-1">
              Completa la información para registrar un nuevo perfil en el sistema.
            </p>
          </div>
          ...
          <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
            ...
            <div className="flex items-center gap-3 px-6 py-4 border-b border-gray-100 bg-gray-50/60">
              ...
              <div className="flex-1 min-w-0">
                <h2 className="text-sm font-semibold text-gray-900">Permisos Adicionales</h2>
                <p className="text-xs text-gray-500 mt-0.5">
                  Selecciona un rol arriba para ver qué permisos incluye. Puedes agregar accesos extra para este usuario.
                </p>
              </div>
